...
func ResourceOrganization() *schema.Resource {
	return &schema.Resource{
		...
		Exists: resourceOrganizationExists,
...
func resourceOrganizationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil && err.Error() == "409 Conflict" {
		return fmt.Errorf("Error: A Grafana Organization with the name '%s' already exists.", name)
	}
...
func resourceOrganizationExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	client := meta.(*gapi.Client)
	orgID, _ := strconv.ParseInt(d.Id(), 10, 64)
...
func resourceOrganizationImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	exists, err := resourceOrganizationExists(d, meta)
	if err != nil || !exists {
		return nil, fmt.Errorf("Error: Unable to import Grafana Organization: %s.", err)
	}
	d.Set("admin_user", "admin")
	d.Set("create_users", "true")
	err = resourceOrganizationRead(d, meta)
	if err != nil {
		return nil, err
	}
